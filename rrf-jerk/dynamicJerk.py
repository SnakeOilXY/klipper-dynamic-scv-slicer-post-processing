...
modifiedGcode.append(GCODE_CMD.replace("{JERK_VALUE}",config[PROFILE_NAME]["default"]).replace("{TYPE}","default"))
for line in srcGcode:
    for key, value in lineTypes.items():
        if line.startswith(value):
            modifiedGcode.append(GCODE_CMD.replace("{JERK_VALUE}",config[PROFILE_NAME][key]).replace("{TYPE}",key))
            break
